...
# nmcli: WiFi setup from within the kiosk app (gui.py WifiScreen)
# python3: the kiosk itself runs as sudo for rpi_ws281x DMA (LED strip)
echo "$RUN_USER ALL=(ALL) NOPASSWD: /usr/bin/nmcli, /usr/bin/python3" | sudo tee "$SUDOERS_FILE" >/dev/null
...
# if it crashes, but stops gracefully when it exits cleanly (exit code 0).
# A clean exit means the user chose "Return to Desktop" from the settings
# screen; anything else (crash, kill) is treated as a fault and restarted.
# Logs to kiosk.log next to this script since there's no systemd journal
# for a desktop-autostarted process.
cd "$(dirname "$0")"
...
while true; do
    ...
    echo "=== $(date) : starting run.py ===" >> "$LOG" 2>&1
    # sudo is required for rpi_ws281x DMA access (LED strip).
    # -u: unbuffered stdout/stderr so messages reach the log immediately.
    sudo python3 -u run.py >> "$LOG" 2>&1
    EXIT_CODE=$?
    echo "=== $(date) : run.py exited (code $EXIT_CODE) ===" >> "$LOG" 2>&1
    if [ "$EXIT_CODE" -eq 0 ]; then
        echo "=== $(date) : clean exit — bartender stopped, returning to desktop ===" >> "$LOG"
        break
    fi
    echo "=== $(date) : crash (code $EXIT_CODE) — restarting in 2s ===" >> "$LOG" 2>&1
    sleep 2
done
EOF
...
# ---------------------------------------------------------------------------
# Step 10: Desktop shortcut — lets the user restart the bartender from the
#   Raspberry Pi desktop after using "Return to Desktop" from the settings
#   screen. Double-clicking the icon relaunches kiosk_launch.sh, which
#   starts run.py in the same crash-recovery loop as at boot.
# ---------------------------------------------------------------------------
header "Step 10: Installing desktop restart shortcut"
DESKTOP_DIR="$HOME/Desktop"
mkdir -p "$DESKTOP_DIR"
cat > "$DESKTOP_DIR/bartender.desktop" <<EOF
[Desktop Entry]
Type=Application
Name=Start Bartender
Comment=Launch the Smart Bartender kiosk
Exec=/bin/bash $LAUNCH_SCRIPT
Icon=applications-other
Terminal=false
Categories=Utility;
EOF
chmod +x "$DESKTOP_DIR/bartender.desktop"
# Some desktop environments require the file to be marked trusted before it
# will run without a security prompt. gio is available on RPi OS Desktop.
gio set "$DESKTOP_DIR/bartender.desktop" metadata::trusted true 2>/dev/null || true
success "Desktop shortcut installed at $DESKTOP_DIR/bartender.desktop"

# ---------------------------------------------------------------------------
# Step 11: Power button service — a completely separate systemd unit from
...
# ---------------------------------------------------------------------------
header "Step 11: Installing power button service"
...
# ---------------------------------------------------------------------------
# Step 12: Verify — run.py already falls back to web-only mode when no
